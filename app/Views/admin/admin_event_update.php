      ...
      <div class="row">
        <div class="col-xs-12 col-md-12">
          <div class="box box-primary">
			...
			<!-- Work Area Start-->
			   <form role="form" action="../updated_event/<?php echo $row->n_id; ?>" enctype="multipart/form-data" method="post">
			   <div class="box-body">
						...
						<div class="form-group col-md-6">
							<label>Title</label>
              <?php if($validation->getError('n_title')) { ?>
                  <label style="color:red;">
                  <?= $error = $validation->getError('n_title'); ?>
                  </label>
                  <?php } ?>
							
							<input type="text" name="n_title" class="form-control" value="<?php echo $row->n_title;?>">							
						...
						<div class="form-group col-md-6">
							<label>Date</label>
              <?php if($validation->getError('n_date')) { ?>
                  <label style="color:red;">
                  <?= $error = $validation->getError('n_date'); ?>
                  </label>
                  <?php } ?>
							
							<input type="date" name="n_date" class="form-control" value="<?php echo $row->n_date;?>">							
						</div>
						...
						<div class="form-group col-md-6">
							<label>Twitter</label>
							<input type="text" name="n_twitter" class="form-control" value="<?php echo $row->n_twitter;?>">							
						</div>
						...
						<div class="form-group col-md-6">
							...
							<input type="text" name="n_fb" class="form-control" value="<?php echo $row->n_fb;?>">							
						
					</div>
					<div class="form-group col-md-6">
							<label>Description</label>
              <?php if($validation->getError('n_desc')) { ?>
                  <label style="color:red;">
                  <?= $error = $validation->getError('n_desc'); ?>
                  </label>
                  <?php } ?>
							
							<textarea name="n_desc"  class="form-control ckeditor" rows=5><?php echo $row->n_desc;?></textarea>
							 						
						</div>

					<div class="form-group col-md-6">
							<label>Priority</label>
							<?php $priority=$row->n_priority;?>
							<select  class="form-control" name="n_priority" >
			  ...
			  <!-- /.box-body -->
			      <div class="box-footer">
					 <a  href="../getData_event" class="btn btn-danger"> << Back</a>
         ...
         <form role="form" action="../updated_event_image/<?php echo $row->n_id; ?>" enctype="multipart/form-data" method="post">
			   <div class="box-body">
         <div class="form-group col-md-6">
							<label>Present Image</label><br>
							<?php if($validation->getError('new_img')) { ?>
                  <label style="color:red;">
                  <?= $error = $validation->getError('new_img'); ?>
                  </label>
                  <?php } ?>
                  <?php if(empty($row->n_img)) { ?>
                      <img src="<?php echo base_url('assest/user/images/defaultImage.jpg'); ?>" width="220px" height="150px"  >
                  <?php } else { ?>
                    <img src="<?php echo base_url('assest/user/images/event/').$row->n_img; ?>"  width="220px" height="150px" >
                  <?php } ?>
							<br> 
							<label>Upload New  Image </label>(Maximum Image Size 512KB)<br>
							<input type="hidden"  name="oldimage" value="<?php echo $row->n_img; ?>">		
							<input type="file" name="new_img" class="form-control" accept="image/*" required>						 				
							<br>
              <div class="box-footer">
              <a  href="../getData_event" class="btn btn-danger"> << Back</a>
              <input type="hidden" name="n_id" value="<?php echo $row->n_id;?>">
